...
namespace CSV_CourseSchedule_PostJob
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                else //reading Oracle
                {
                    ...
                    List<DLL.Models.OrganizationCampusBuildingRoom> dbRooms = new List<DLL.Models.OrganizationCampusBuildingRoom>();
                    List<DLL.Models.OrganizationProgramShift> dbShifts = new List<DLL.Models.OrganizationProgramShift>();
                    ...
                    //DateTime dtStartLoop = DateTime.Now; DateTime dtEndLoop = DateTime.Now;
                    List<CMSClassSchedule> enrCMSList = new List<CMSClassSchedule>();

                    enrCMSList = CMS_GetScheduleRowUnsyncedYet(strORCCMSConnString);
                    if (enrCMSList != null && enrCMSList.Count > 0)
                    {
                        foreach (var enrCMS in enrCMSList)
                        {
                            DateTime dtStartLoop = DateTime.ParseExact(enrCMS.Start_Date, "MM/dd/yyyy", CultureInfo.CurrentCulture);
                            DateTime dtEndLoop = DateTime.ParseExact(enrCMS.End_Date, "MM/dd/yyyy", CultureInfo.CurrentCulture);
                            ...
                            if (weeks_count > 0)
                            {
                                for (int i = 0; i < weeks_count; i++)
                                {
                                    ...
                                    DateTime dtStartTime = DateTime.ParseExact(enrCMS.Start_Date + " " + enrCMS.Mtg_Start, "MM/dd/yyyy hh:mm:ss tt", CultureInfo.CurrentCulture);
                                    DateTime dtEndTime = DateTime.ParseExact(enrCMS.Start_Date + " " + enrCMS.Mtg_End, "MM/dd/yyyy hh:mm:ss tt", CultureInfo.CurrentCulture);

                                    //get campus id using CMS code
                                    var dbCampus = dbCampuses.Where(c => c.CampusCode.ToLower() == enrCMS.Campus.ToLower()).FirstOrDefault();
                                    int iCampusID = dbCampus != null ? dbCampus.Id : 0;

                                    //get course id using CMS code
                                    var dbCourse = dbCourses.Where(c => c.CourseCode.ToLower() == enrCMS.Course_Id.ToLower()).FirstOrDefault();
                                    int iCourseID = dbCourse != null ? dbCourse.Id : 0;

                                    //get program id using CMS code
                                    var dbProgram = dbPrograms.Where(c => c.ProgramCode.ToLower() == enrCMS.Catalog.ToLower()).FirstOrDefault();
                                    int iProgram = dbProgram != null ? dbProgram.Id : 0;

                                    //get room id using CMS code
                                    var dbRoom = dbRooms.Where(c => c.TerminalId.ToString() == enrCMS.Term.ToLower()).FirstOrDefault();
                                    ...
                                    using (var db2 = new DLL.Models.Context())
                                    {
                                        ...
                                        if (enrCMS.Mon.ToUpper() == "Y")
                                        {
                                            if (dtStartTime.AddDays(current_day).DayOfWeek.ToString().ToUpper() == "MONDAY")
                                            {
                                                ...
                                                CheckAlreadyExistsThenDelete(strORCCMSConnString, iCampusID, iCourseID, iProgram, sch.StartTime);
                                        ...
                                        if (enrCMS.Sun.ToUpper() == "Y")
                                        {
                                            if (dtStartTime.AddDays(current_day).DayOfWeek.ToString().ToUpper() == "SUNDAY")
                                            {
                                                ...
                                                //check already exists then delete
                                                CheckAlreadyExistsThenDelete(strORCCMSConnString, iCampusID, iCourseID, iProgram, sch.StartTime.Date);
        ...
        public static void CheckAlreadyExistsThenDelete(string strConnString, int cam_id, int crs_id, int prg_id, DateTime dt)
        {
            using (var db = new DLL.Models.Context())
            {
                var dbOrgCourseSchedule = db.organization_campus_room_course_schedule.Where(s => s.CampusId == cam_id && s.CourseId == crs_id && s.ProgramId == prg_id && (s.StartTime.Day == dt.Day && s.StartTime.Month == dt.Month && s.StartTime.Year == dt.Year)).FirstOrDefault();
        ...
        public static List<CMSClassSchedule> CMS_GetScheduleRowUnsyncedYet(string conString)
        {
            List<CMSClassSchedule> enrCMSList = new List<CMSClassSchedule>();
            ...
            cmd.CommandText = "select * from intr_cms_class_schedules order by id";
            ...
            if (_SqlDataReader != null && _SqlDataReader.HasRows)
            {
                ...
                while (_SqlDataReader.Read())
                {
                    ...
                    enrCMS.Career = _SqlDataReader["CAREER"].ToString() ?? "";
                    enrCMS.Descr = _SqlDataReader["DESCR"].ToString() ?? "";
                    enrCMS.Catalog = _SqlDataReader["CATALOG"].ToString() ?? "";
                    enrCMS.Acad_Group = _SqlDataReader["ACAD_GROUP"].ToString() ?? "";
                    enrCMS.Term = _SqlDataReader["TERM"].ToString() ?? "";
                    ...
                    enrCMS.Start_Date = _SqlDataReader["START_DATE"].ToString() ?? "";
                    enrCMS.End_Date = _SqlDataReader["END_DATE"].ToString() ?? "";
