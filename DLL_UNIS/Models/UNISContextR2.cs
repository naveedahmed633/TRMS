namespace DLL_UNIS.Models
{
    ...
    /// <summary>Second UCDB connection (same schema as UNISContext). Existing database — no EF migrations.</summary>
    public partial class UNISContextR2 : DbContext
    {
        public static string connectionStringR2 = GetConnectionString(ConfigurationManager.ConnectionStrings["UNISContextR2"].ConnectionString.Trim());

        static UNISContextR2()
        {
            Database.SetInitializer<UNISContextR2>(null);
        }

        public UNISContextR2()
            : base(connectionStringR2)
        {
        }
