...
(function($) {
	...
	$.fn.timepicki = function(options) {
	    var theInputField = this;
		var defaults = {
		    format_output: function (tim, mini, meri) {

		        function isValidTime(is24hr, hours, mins) {
		            // if either hours or minutes are invalid,
                    // reutrn false.
		            ...
		            if (!is24hr) {
		                ...
		                // for 12 hour clock,
		                // check if, hours are between 1 and 12 (inclusive)
                        // and minutes are between 0 and 59 (inclusive).
		                if( (hrs >= 1 && hrs <= 12) &&
                            (min >= 0 && min <= 59)) {
		                    return true;
		                }
		                ...
		                // for 24 hours
		                // check if the hours are between 0 and 23 (inclusive),
		                // and minutes are between 0 and 59 (inclusive).
		                if ((hrs >= 0 && hrs <= 23) &&
                            (min >= 0 && min <= 59)) {
		                    return true;
			    ...
			    if (settings.show_meridian) {
			        ...
			        var lastValue = $(theInputField).val();

			        if (isValidTime(false, tim, mini)) {
		...
		return this.each(function() {
			...
			var top_arrow_button = (settings.increase_direction === 'down') ?
				"<div class='tp_prev action-prev'></div>" :
				"<div class='tp_prev action-next'></div>";
			var bottom_arrow_button = (settings.increase_direction === 'down') ?
				"<div class='tp_next action-next'></div>" :
				"<div class='tp_next action-prev'></div>";
			...
			function change_time(cur_ele, direction) {
				...
				if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'tp_next') {
				...
				} else if ((cur_ele && cur_ele.hasClass('action-prev')) || direction === 'tp_prev') {
