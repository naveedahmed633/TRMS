...
Helpers =
{
    ...
    removeAllClasses: function (elementSelector) {
        ...
        $(elementSelector).each(function (i) {
            ...
            $(this).removeClass('employee-override');
    ...
    assignEmployeeOverride: function (calendarOverrideObject) {
        ...
        // only assign if the employee override is for this year
        if (calendarData.year != parseInt(year)) {
            return;
        }
        ...
        switch (calendarOverrideObject.type) {
            ...
            case "holiday":
                $(dayElement).addClass("employee-override");
                Helpers.applyPopOver(dayElement, "Holiday", calendarOverrideObject.reason);
                break;
...
RenderingFunctions = {
    repaintCalendar: function () {
        ...
        function getShiftIndex(shiftId) {
            ...
            // The control will only reach here, if the shift for id 'shiftId'
            // is not in the availableShifts list. Which means that a previously
            // assigned shift was remove from this group.
            alert("Unable to display Calendar!\n    A human error has occured. One of the shifts which was previously assigned to your group has been taken away.\n Please contact HR");

            $('#toast').fadeOut(100);

            $("#calendar_edit_form").hide();
        ...
        /********************************** Render Employee Overrides ******************************************/

        if (employeeCalendarOverrides.length > 0) {
            // loop over all the overrides and assign colors
            // and pop overs. TODO
            for (var i = 0; i < employeeCalendarOverrides.length; i++) {

                Helpers.assignEmployeeOverride(employeeCalendarOverrides[i]);
...
CalendarUtils =
    {
        submitEmployeeCalendar: function () {
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    overrides: employeeCalendarOverrides
                }),
                success: function (data) {
                    ...
                    if (data.success == true) {
                        ...
                        alert('There was an error. the database server might be down.');
        ...
        /*getGeneralCalendar('2014', 'http://localhost:57588/LM/ShiftManagement/getCalendarForYear', function (calendar) {
        ...
        getGeneralCalendar: function (yearForCalendar, calendarURL, callback) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (calendarObject.isGeneralCalendar) {
                        thisIsGeneralCalendar = true;
                    } else {
                        thisIsGeneralCalendar = false;
                    }
        ...
        /*
        getGeneralShifts("http://localhost:57588/LM/ShiftManagement/getShiftsForGeneralCalendar",function (shifts) {
          console.log(shifts);
        });
        */
...
/**************************************************************
/ This is the most important function for ManageEmployeeCalendar
/ view. This function defines what happens when the Line manager 
/ presses the day element to add a shift for the selected employee
...
function initEditDayModal(parentDivIdentifier) {
            ...
            <div class='container-fluid'>\
                <div class='row'>\
                    <div class='col-md-12'>\
                       ...
                       <div class='row'>\
                        <div class='col-4'>\
                            <label class='control-label' style='width: 200px;'>From Date</label>\
                            <input class='form-control'  id='dt_from' type='date' name='form_date' style='width: 200px;'>\
                        </div>\
                         </div>\
                            <div class='row'>\
                                <div class='col-4'>\
                                    <label class='control-label' style='width: 200px;'>To Date</label>\
                                    <input class='form-control'  id='dt_to' type='date' name='to_date' style='width: 200px;'>\
                                </div>\
                            </div>\
                                    \
    ...
    function onClickDone() {
        ...
        var todate = $('#dt_to').val();
        var fromdate = $('#dt_from').val();
        var n_to_date = new Date(todate);
        var n_from_date = new Date(fromdate);
        var newtodatess = moment(n_to_date).format('MM/DD/YYYY');
        var newfromdate = moment(n_from_date).format('MM/DD/YYYY');

        var itr = moment.twix(newfromdate, newtodatess).iterate("days");
        var range = [];
        while (itr.hasNext()) {
        ...
        for (var j = 0; j < range.length; j++) {
            ...
            for (var i = 0; i < employeeCalendarOverrides.length; i++) {
                if (employeeCalendarOverrides[i].date == dd) {//editDayFormDate
                    employeeCalendarOverrides[i] = obj;
            ...
            employeeCalendarOverrides[employeeCalendarOverrides.length] = obj;
            RenderingFunctions.repaintCalendar();
    ...
    toReturn = function (dayElement) {
        ...
        function loadDataIfAlreadyOverriden(date) {
            ...
            var generalOverrides = employeeCalendarOverrides;
        ...
        ////////////////////// IR Update //////////////////////////////////////
        var getdate = $("#edit-day-date-title").text();
        var sd = moment(getdate).format('YYYY-MM-DD');
        $("#dt_to").val(sd);
        $("#dt_from").val(sd);
        ///////////////////////////////////////////////////////////////////////
...
function initializeTimeTuneGeneralCalendar() {
    ...
    $.fn.calendar().__proto__._render = function () {
        ...
        CalendarUtils.getGeneralCalendar(calendarData.year, '/LM/ShiftManagement/getGeneralCalendarForYear', function (calendar) {

            if (calendar == null) {

                $('#toast').fadeOut(600);

                alert("TimeTune: We're sorry! no calendar found.\nIt might be because you have not been assigned to a group. \nPlease contact HR.");
                return;
            }
            ...
            // Open edit day modal on clicking a 
            // day element.
            $(".day-content").on("click", function (e) {
                //alert("I'm here");
                //console.log();
                //if (!thisIsGeneralCalendar)
                if (typeof SELECTED_EMPLOYEE != 'undefined')
...
function loadShifts() {
    ...
    $('#toast').html('Please Wait!<br><br>Loading shifts....');
    $('#toast').fadeIn(400);
    // fetch shifts first
    CalendarUtils.getGeneralShifts("/LM/ShiftManagement/getShiftsForEmployeeCalendar", function (shifts) {

        $('#toast').fadeOut(600);
        if (shifts.length == 0) {
            alert("TimeTune: Your group has not been assigned any shifts, please contach HR.");
            return;
        }
        ...
        /*
        //***************************************************************
        // we will not have any drop downs because this is the employee *
        // calendar and not the group calendar.                         *
        //***************************************************************
...
// its like a call back in loadShifts(); which does
...
function setSubmitButtonAction() {
    ...
    $('#save_calendar_button').on("click", function (e) {

        CalendarUtils.submitEmployeeCalendar();
...
$(document).ready(function () {
    ...
    // load the shifts that are required for the calendar to display.
    loadShifts();
...
function loadEmployeeCalendar() {
    if (typeof SELECTED_EMPLOYEE != 'undefined') {
        $.ajax({
            type: 'POST',
            url: "/LM/ShiftManagement/getEmployeeCalendar",
            data: {
                employee_id: SELECTED_EMPLOYEE,
            ...
            success: function (data) {
                ...
                if (data.success) {
                    employeeCalendarOverrides = data.data;

                    RenderingFunctions.repaintCalendar();

                    console.log(employeeCalendarOverrides);
                }
