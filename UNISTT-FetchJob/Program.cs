...
namespace UNISTT_FetchJob
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                if (ConfigurationManager.AppSettings["LogTitle"] != null && ConfigurationManager.AppSettings["LogTitle"].ToString() != "")
                {
                    strLTitleText = ConfigurationManager.AppSettings["LogTitle"].ToString();
                }
                ...
                strStep = "1";
                Console.WriteLine("U-T: Get UNISTimeTune DB Active Data Only");
                ...
                strStep = "2";
                Console.WriteLine("U-T: INSERT INTO TimeTune DB");
                ...
                strStep = "3"; cutOff = 0;
                Console.WriteLine("U-T: Inactive UNISTimeTune Data");
                ...
                Console.WriteLine("U-T: ALL Steps Done Successfully!");
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        ...
                        {
                            LogTitle = "U-T: ALL Steps Done " + strLTitleText,
                            LogDateTime = DateTime.Now,
                            LogMessageText = "U-T: ALL Steps Done " + strLTitleText + " - Succeeded! (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + ")"
                        });

                        db.SaveChanges();
                        Console.WriteLine("U-T: ALL Steps Done " + strLTitleText + " - Message Logged with Success!");
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "U-T: Exception " + strLTitleText,
                            LogDateTime = DateTime.Now,
                            LogMessageText = "U-T: Exception " + strLTitleText + " - At STEP: " + strStep + " (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + "), StackTrace: " + stkText + ", Message: " + msgText + ", Inner: " + innText
                        });

                        db.SaveChanges();
                        Console.WriteLine("U-T: Exception " + strLTitleText + " - At STEP: " + strStep + " (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + "), StackTrace: " + stkText + ", Message: " + msgText + ", Inner: " + innText);
                    }
                }
            }
