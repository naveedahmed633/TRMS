...
function initEditModal(parentDivIdentifier, updateURL, capital, bld_list, trm_list) {
    ...
    var HTML = "\
        ...
        <form>\
            ...
            <select id='modal_" + capital + "_building_id' name='BuildingId' class='form-control'>" + generateDDLOptions(bld_list) + "</select>\
            <label class='control-label'>Room Code</label>\
            <input type='text' class='form-control' name='RoomCode' id='modal_" + capital + "_room_code' />\
            <label class='control-label'>Room Title</label>\
            <input type='text' name='RoomTitle' id='modal_" + capital + "_room_title' class='form-control' />\
            <label class='control-label'>Terminal</label>\
            <select id='modal_" + capital + "_terminal_id' name='TerminalId' class='form-control'>" + generateDDLOptions(trm_list) + "</select>\
            <label class='control-label'>Floor Number</label>\
            <select id='modal_" + capital + "_floor_number' name='FloorNumber' class='form-control'>\
    ...
    var fieldBuildingID = $("#modal_" + capital + "_building_id");
    var fieldRoomCode = $("#modal_" + capital + "_room_code");
    var fieldRoomTitle = $("#modal_" + capital + "_room_title");
    var fieldTerminalID = $("#modal_" + capital + "_terminal_id");
    var fieldFloorNumber = $("#modal_" + capital + "_floor_number");

    //console.log("fieldStatus = " + fieldStatus.val());
    var allFields = $([]).add(fieldID).add(fieldBuildingID).add(fieldRoomCode).add(fieldRoomTitle).add(fieldTerminalID).add(fieldFloorNumber);
        ...
        if (valid) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    BuildingId: parseInt(fieldBuildingID.val()),
                    RoomCode: fieldRoomCode.val(),
                    RoomTitle: fieldRoomTitle.val(),
                    TerminalId: parseInt(fieldTerminalID.val()),
                    FloorNumber: parseInt(fieldFloorNumber.val())
                },
                success: function (data) {
                    ...
                    if (data.status == "success") {
                        ...
                        $('*[data-row="' + rowBeingEdited + '"]').parent().parent().find('td:eq(1)').html(fieldBuildingID.val() + "*");
                        $('*[data-row="' + rowBeingEdited + '"]').parent().parent().find('td:eq(2)').html(fieldRoomCode.val());
                        $('*[data-row="' + rowBeingEdited + '"]').parent().parent().find('td:eq(3)').html(fieldRoomTitle.val());
                        $('*[data-row="' + rowBeingEdited + '"]').parent().parent().find('td:eq(4)').html(fieldTerminalID.val() + "*");
                        $('*[data-row="' + rowBeingEdited + '"]').parent().parent().find('td:eq(5)').html(fieldFloorNumber.val());
    ...
    toReturn = function (id, bld_id, room_code, room_title, terminal_id, floor_number) {

        $('#modal_' + capital + '_id').val(id);
        $('#modal_' + capital + '_building_id').val(bld_id);
        $('#modal_' + capital + '_room_code').val(room_code);
        $('#modal_' + capital + '_room_title').val(room_title);
        $('#modal_' + capital + '_terminal_id').val(terminal_id);
        $('#modal_' + capital + '_floor_number').val(floor_number);
        ...
        $('#modal_' + capital + '_bld_id').focus();
    };

    // When the enter key is pressed
    // prevent the form from being submitted.
    editDialog.find("form").on("submit", function (id, bld_id, room_code, room_title, terminal_id, floor_number) {
