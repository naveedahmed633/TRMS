...
namespace MvcApplication1.Areas.LM.Controllers
{
    ...
    {  //
       // GET: /LM/LM/
        ...
        private DashboardManagerData GetDataForDashboard()
        {
            string strAttendanceChartData = "", strLeavesChartData = "";

            ViewModels.DashboardManagerData dashboardData = new ViewModels.DashboardManagerData();
            dashboardData.lineManager = TimeTune.Dashboard.getDashboardValues(User.Identity.Name);
            // var dashboardValues = TimeTune.Dashboard.getDashboardValuesOfEmployee(User.Identity.Name);

            //  dashboard.employee.dashboardElement = dashboardValues.dashboardElement;
            //  dashboard.employeeCount = dashboardValues.employeeCount;
            //  dashboard.lineManagerName = dashboardValues.supervisorName;  

            if (dashboardData.lineManager.dashboardElement != null)
            {
                decimal sumPresence = 0, sumAvaiabilityPunc = 0, sumAvaiabilityExit = 0;
                sumPresence = dashboardData.lineManager.dashboardElement.present + dashboardData.lineManager.dashboardElement.leave + dashboardData.lineManager.dashboardElement.absent;
                sumAvaiabilityPunc = dashboardData.lineManager.dashboardElement.onTime + dashboardData.lineManager.dashboardElement.late;
                sumAvaiabilityExit = dashboardData.lineManager.dashboardElement.onTime + dashboardData.lineManager.dashboardElement.earlyGone;
                ...
                if (sumPresence != 0)
                {
                    perPresent = Math.Round(Math.Floor(dashboardData.lineManager.dashboardElement.present / sumPresence * 100));
                    perLeave = Math.Round(Math.Floor(dashboardData.lineManager.dashboardElement.leave / sumPresence * 100));
                    perAbsent = Math.Round(Math.Ceiling(dashboardData.lineManager.dashboardElement.absent / sumPresence * 100));
                }

                if (sumAvaiabilityPunc != 0)
                {
                    perOnTime = Math.Round(Math.Floor(dashboardData.lineManager.dashboardElement.onTime / sumAvaiabilityPunc * 100));
                    perLate = Math.Round(Math.Floor(dashboardData.lineManager.dashboardElement.late / sumAvaiabilityPunc * 100));
                }

                if (sumAvaiabilityExit != 0)
                {
                    perOutTime = Math.Round(Math.Floor(dashboardData.lineManager.dashboardElement.onTime / sumAvaiabilityExit * 100));
                    perEarlyGone = Math.Round(Math.Ceiling(dashboardData.lineManager.dashboardElement.earlyGone / sumAvaiabilityExit * 100));
                }
            ...
            var leaves = TimeTune.Dashboard.getDashboardLeavesForLM(User.Identity.Name);
            ...
            //events list
            var events = TimeTune.Dashboard.getDashboardEventsForLM(User.Identity.Name);
        ...
        public string DashboardPALChartDataMyTeam()
        {
            ...
            present = Session["LMPresentCounter"] != null ? float.Parse(Session["LMPresentCounter"].ToString()) : 0.0f;
            leave = Session["LMLeaveCounter"] != null ? float.Parse(Session["LMLeaveCounter"].ToString()) : 0.0f;
            absent = Session["LMAbsentCounter"] != null ? float.Parse(Session["LMAbsentCounter"].ToString()) : 0.0f;
            ...
            ontime = Session["LMOnTimeCounter"] != null ? float.Parse(Session["LMOnTimeCounter"].ToString()) : 0.0f;
            late = Session["LMLateCounter"] != null ? float.Parse(Session["LMLateCounter"].ToString()) : 0.0f;
            early = Session["LMEarlyCounter"] != null ? float.Parse(Session["LMEarlyCounter"].ToString()) : 0.0f;
        ...
        {
            //Session["LMPresentCounter"] = 0;
            //Session["LMLeaveCounter"] = 0;
            //Session["LMAbsentCounter"] = 0;

            //Session["LMOnTimeCounter"] = 0;
            //Session["LMLateCounter"] = 0;
            //Session["LMEarlyCounter"] = 0;
            ...
            ViewModels.DashboardManager dashboard = new ViewModels.DashboardManager();
            //var model = TimeTune.Dashboard.getSupervisor(User.Identity.Name);
            TimeTune.Dashboard.LMAttendanceCount model = new TimeTune.Dashboard.LMAttendanceCount();

            model.getSupervisorData = TimeTune.Dashboard.getSupervisor(User.Identity.Name);
            model.data = TimeTune.Dashboard.getTodayMyTeamValues(User.Identity.Name);
            return View(model);
        ...
        public ActionResult SupervisorTeamPDF()
        {
            ...
            count = TimeTune.Dashboard.generatePDF_LM("", User.Identity.Name, out dtsource);
        ...
        public JsonResult DataHandler(DTParameters param)
        {
            ...
            try
            {
                ...
                // get all employee view models
                count = TimeTune.Dashboard.searchEmployeeForLMTeam(param.Search.Value, param.SortOrder, User.Identity.Name, param.Start, param.Length, out dtsource);

                if (count == 0)
                {
                    count = TimeTune.Dashboard.searchEmployeeDashboardSLM(param.Search.Value, param.SortOrder, User.Identity.Name, param.Start, param.Length, out dtsource);
                }
