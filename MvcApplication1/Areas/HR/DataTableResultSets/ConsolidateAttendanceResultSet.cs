...
namespace MvcApplication1
{
    public class ConsolidateAttendanceResultSet
    {
        public static List<ConsolidatedAttendanceLog> GetResult(string search, string sortOrder, int start, int length, List<ConsolidatedAttendanceLog> dtResult)
        {
            ...
            //{
            //    try { 
            //        DateTime current = DateTime.ParseExact(toReturn[i].date, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture); //DateTime.Parse(toReturn[i].date);
            //        toReturn[i].date = current.ToString("dd-MM-yyyy");
            //    }
            //    catch
            //    {
            //        // Dear rubberduck,
        ...
        private static IQueryable<ConsolidatedAttendanceLog> FilterResult(string search, List<ConsolidatedAttendanceLog> dtResult)
        {
            ...
            //if (search != null && search.Split(':').Count() == 2 && search.Length <= 8) //if (search != null && search.Split(':').Count() == 3)
            //{
            //    times = DateTime.ParseExact(search, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture); //DateTime.Parse(search);
            ...
            results = results.Where(p =>
                (p.employee_code != null && (
                    ...
                    (p.employee_code.ToLower().Contains(search.ToLower()))
                        ...
                        || (p.date.ToLower().Contains(search.ToLower()))
                        || (p.time_in.ToLower().Contains(sdd.ToLower()))
                        || (p.time_out.ToLower().Contains(search.ToLower()))
