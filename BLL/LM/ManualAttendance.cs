...
namespace TimeTune
{
    ...
    {
        public class EmployeesInfoByHierarchy
        {
        ...
        public static List<ViewModels.Employee> getEmployeeBySupervisorRecursion(string supervisorCode)
        {
            List<EmployeesInfoByHierarchy> toReturn = new List<EmployeesInfoByHierarchy>();
            List<ViewModels.Employee> listEmployees = new List<ViewModels.Employee>();

            using (Context db = new Context())
            {
                toReturn = db.Database.SqlQuery<EmployeesInfoByHierarchy>(string.Format("SP_GetEmployeesByRecursion @Employee_Code='{0}'", supervisorCode)).ToList();
                if (toReturn != null && toReturn.Count > 0)
                {
                    foreach (var e in toReturn)
                    {
                        listEmployees.Add(new ViewModels.Employee
                        {
                            id = e.EmployeeID,
                            employee_code = e.EmployeeCode,
                            first_name = e.EmployeeInfo,
                            last_name = ""
                        });
                    }
                }
        ...
        ////////////////////// Backup /////////////////////////////
        public static List<ViewModels.Employee> getEmployeeBySupervisorNested(string supervisorCode)
        {
            List<DLL.Models.Employee> employee = null;
            List<ViewModels.Employee> toReturn = new List<ViewModels.Employee>();
            ...
            using (Context db = new Context())
            {
                try
                {
                    var dbEmployees = db.employee.Where(m => m.active).ToList();
                    if (dbEmployees != null && dbEmployees.Count > 0)
                    {
                        var supervisor = dbEmployees.Where(m => m.employee_code.Equals(supervisorCode)).SingleOrDefault();
                        if (supervisor != null && supervisor.Group != null)
                        {
                            int id = supervisor.Group.GroupId;

                            employee = dbEmployees.Where(m => m.Group != null && m.Group.GroupId == id).ToList();
                            ...
                            //SLMs
                            var data_slms = db.super_line_manager_tagging.Where(s => s.superLineManager.EmployeeId == supervisor.EmployeeId).ToList();
                            if (data_slms != null && data_slms.Count > 0)
                            {
                                foreach (var s in data_slms)
                                {
                                    var data_semp = dbEmployees.Where(m => m.EmployeeId == s.taggedEmployee.EmployeeId).ToList();
                                    if (data_semp != null && data_semp.Count > 0)
                                    {
                                        for (int i = 0; i < data_semp.Count(); i++)
                                        {
                                            ViewModels.Employee employees = new ViewModels.Employee();
                                            DLL.Models.Employee currentEmployee = data_semp[i];
                                            employees.id = currentEmployee.EmployeeId;
                                            employees.first_name = currentEmployee.first_name;
                                            employees.last_name = currentEmployee.last_name;
                                            employees.employee_code = currentEmployee.employee_code;
                                ...
                                foreach (var s in data_slms)
                                {
                                    ...
                                    if (data_semp != null && data_semp.Count > 0)
                                    {
                                        for (int i = 0; i < data_semp.Count(); i++)
                                        {
                                            ...
                                            //*/EMPs under SLMs
                                            int sID = 0; sID = data_semp[i].EmployeeId;

                                            if (sID != supervisor.EmployeeId)
                                            {
                                                List<ViewModels.Employee> listEMPs = new List<ViewModels.Employee>();
                                                listEMPs = GetLMsForSLMsNested(dbEmployees, sID);
                                                if (listEMPs != null && listEMPs.Count > 0)
                                                {
                                                    toReturn.AddRange(listEMPs);
                                                }
                                            }
                                            //*/
                                        }
                ...
                ////toReturn = toReturn.OrderBy(o => o.employee_code).ToList();
            }

            return toReturn;
        }

        public static List<ViewModels.Employee> GetLMsForSLMsNested(List<Employee> dbEmployees, int sID)
        {
            List<ViewModels.Employee> toReturn = new List<ViewModels.Employee>();
            List<DLL.Models.Employee> employee = null;

            var supervisorUnder = dbEmployees.Where(m => m.EmployeeId == sID).SingleOrDefault();
            if (supervisorUnder != null && supervisorUnder.Group != null)
            {
                int id = supervisorUnder.Group.GroupId;
                ...
                if (employee != null && employee.Count > 0)
                {
                    ...
                    {
                        if (employee[j].EmployeeId != sID)
                        {
                            ViewModels.Employee employeesUnder = new ViewModels.Employee();
                            DLL.Models.Employee currentEmpUnder = employee[j];
                            employeesUnder.id = currentEmpUnder.EmployeeId;
                            employeesUnder.first_name = currentEmpUnder.first_name;
                            employeesUnder.last_name = currentEmpUnder.last_name;
                            employeesUnder.employee_code = ">>" + currentEmpUnder.employee_code;
                            employeesUnder.email = currentEmpUnder.email;
                            employeesUnder.address = currentEmpUnder.address;
                            employeesUnder.mobile_no = currentEmpUnder.mobile_no;
                            employeesUnder.date_of_joining =
                                (currentEmpUnder.date_of_joining != null && currentEmpUnder.date_of_joining.HasValue)
                                ? currentEmpUnder.date_of_joining.Value.ToString("dd-MM-yyyy") : "";
                            employeesUnder.date_of_leaving =
                                (currentEmpUnder.date_of_leaving != null && currentEmpUnder.date_of_leaving.HasValue)
                                ? currentEmpUnder.date_of_leaving.Value.ToString("dd-MM-yyyy") : "";
                            employeesUnder.function_id = (currentEmpUnder.function != null) ? currentEmpUnder.function.FunctionId : -1;
                            employeesUnder.function_name = (currentEmpUnder.function != null) ? currentEmpUnder.function.name : "";

                            employeesUnder.department_id = (currentEmpUnder.department != null) ? currentEmpUnder.department.DepartmentId : -1;
                            employeesUnder.department_name = (currentEmpUnder.department != null) ? currentEmpUnder.department.name : "";

                            employeesUnder.designation_id = (currentEmpUnder.designation != null) ? currentEmpUnder.designation.DesignationId : -1;
                            employeesUnder.designation_name = (currentEmpUnder.designation != null) ? currentEmpUnder.designation.name : "";

                            employeesUnder.access_group_id = (currentEmpUnder.access_group != null) ? currentEmpUnder.access_group.AccessGroupId : -1;
                            employeesUnder.access_group_name = (currentEmpUnder.access_group != null) ? currentEmpUnder.access_group.name : "";

                            employeesUnder.grade_id = (currentEmpUnder.grade != null) ? currentEmpUnder.grade.GradeId : -1;
                            employeesUnder.grade_name = (currentEmpUnder.grade != null) ? currentEmpUnder.grade.name : "";

                            employeesUnder.group_id = (currentEmpUnder.Group != null) ? currentEmpUnder.Group.GroupId : -1;
                            employeesUnder.group_name = (currentEmpUnder.Group != null) ? currentEmpUnder.Group.group_name : "";

                            employeesUnder.region_id = (currentEmpUnder.region != null) ? currentEmpUnder.region.RegionId : -1;
                            employeesUnder.region_name = (currentEmpUnder.region != null) ? currentEmpUnder.region.name : "";

                            employeesUnder.type_of_employment_id = (currentEmpUnder.type_of_employment != null) ? currentEmpUnder.type_of_employment.TypeOfEmploymentId : -1;
                            employeesUnder.type_of_employment_name = (currentEmpUnder.type_of_employment != null) ? currentEmpUnder.type_of_employment.name : "";

                            employeesUnder.location_id = (currentEmpUnder.location != null) ? currentEmpUnder.location.LocationId : -1;
                            employeesUnder.location_name = (currentEmpUnder.location != null) ? currentEmpUnder.location.name : "";

                            toReturn.Add(employeesUnder);


                            ////*/EMPs under SLMs - 2
                            //int eID = 0; eID = employee[j].EmployeeId;

                            //if (eID != sID)
                            //{
                            //    List<ViewModels.Employee> listEMPs = new List<ViewModels.Employee>();
                            //    listEMPs = GetNestedLMsForSLMs(dbEmployees, eID);
                            //    if (listEMPs != null && listEMPs.Count > 0)
                            //    {
                            //        toReturn.AddRange(listEMPs);
                            //    }
