    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                strStep = "1";
                Console.WriteLine("UC-UC: tUser - Get UNISMaster DB Active Data Only");
                ...
                strStep = "2";
                Console.WriteLine("UC-UC: tUser - INSERT INTO 'UNIS Region' DB");
                        ...
                        foreach (var entity in allActivetUsers)
                        {
                            ...
                            if (dbUNIStUser != null)
                            {
                                ...
                                using (var dbTT = new DLL.Models.Context())
                                {
                                    ...
                                    {
                                        LogTitle = "UC-UC: tUser - DELETED " + strLTitleText + "",
                                        LogDateTime = DateTime.Now,
                                        LogMessageText = "UC-UC: tUser - DELETED (Id=" + u_id.ToString() + ")"
                                    });
                                    dbTT.SaveChanges();

                                    Console.WriteLine("UC-UC: tUser - DELETED (Id=" + u_id.ToString() + ")");
                                }
                            }
                ...
                strStep = "3"; cutOff = 0;
                Console.WriteLine("UC-UC: tUser - Inactive Master Data");
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        ...
                        {
                            LogTitle = "UC-UC: tUser - Sync " + strLTitleText + "",
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: tUser - Sync " + strLTitleText + " - Succeeded! (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + ")"
                        });

                        db.SaveChanges();
                        Console.WriteLine("UC-UC: tUser - Sync " + strLTitleText + " - Message Logged with Success!");
                    }
                }
                */
                #endregion
                ...
                strStep = "1";
                Console.WriteLine("UC-UC: tEmploye - Get UNISMaster DB Active Data Only");
                ...
                strStep = "2";
                Console.WriteLine("UC-UC: tEmploye - INSERT INTO 'UNIS Region' DB");
                        ...
                        foreach (var entity in allActivetEmployees)
                        {
                            ...
                            if (dbUNIStEmployee != null)
                            {
                                ...
                                using (var dbTT = new DLL.Models.Context())
                                {
                                    dbTT.log_message.Add(new DLL.Models.LogMessage
                                    {
                                        LogTitle = "UC-UC: tEmployee - DELETED " + strLTitleText + "",
                                        LogDateTime = DateTime.Now,
                                        LogMessageText = "UC-UC: tEmployee - DELETED (Id=" + u_id.ToString() + ")"
                                    });
                                    dbTT.SaveChanges();

                                    Console.WriteLine("UC-UC: tEmployee - DELETED (Id=" + u_id.ToString() + ")");
                                }
                            }
                ...
                strStep = "3"; cutOff = 0;
                Console.WriteLine("UC-UC: tEmploye - Inactive Master Data");
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "UC-UC: tEmploye - Sync " + strLTitleText,
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: tEmploye - Sync " + strLTitleText + " - Succeeded! (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + ")"
                        });

                        db.SaveChanges();
                        Console.WriteLine("UC-UC: tEmploye - Sync " + strLTitleText + " - Message Logged with Success!");
                    }
                }
                */
                #endregion
                ...
                strStep = "1";
                Console.WriteLine("UC-UC: iUserCard - Get UNISMaster DB Active Data Only");

                iUserCardsResco[] allActiveiUserCardsResco = null;
                using (var db = new ContextUNISDataLogMaster())
                {
                    if (strUNISRegionID == "2")
                        allActiveiUserCardsResco = db.UNIS_iUserCardsResco.Where(m => m.IsSynced2 && m.Server_ID == 2).ToArray();
                    else if (strUNISRegionID == "3")
                        allActiveiUserCardsResco = db.UNIS_iUserCardsResco.Where(m => m.IsSynced3 && m.Server_ID == 3).ToArray();
                    else
                        allActiveiUserCardsResco = db.UNIS_iUserCardsResco.Where(m => m.IsSynced && m.Server_ID == 1).ToArray();

                    if (allActiveiUserCardsResco.Length > 0)
                    {
                        count1 = allActiveiUserCardsResco.Length;
                    }
                }
                ...
                strStep = "2";
                Console.WriteLine("UC-UC: iUserCard - INSERT INTO 'UNIS Region' DB");

                if (allActiveiUserCardsResco != null && allActiveiUserCardsResco.Length > 0)
                {
                    using (var db = new ContextUNISRegion())
                    {
                        foreach (var entity in allActiveiUserCardsResco)
                        {
                            //IR Added Code for Existing Data by PK
                            ...
                            if (dbUNISiUserCard != null)
                            {
                                ...
                                using (var dbTT = new DLL.Models.Context())
                                {
                                    dbTT.log_message.Add(new DLL.Models.LogMessage
                                    {
                                        LogTitle = "UC-UC: iUserCard - DELETED " + strLTitleText + "",
                                        LogDateTime = DateTime.Now,
                                        LogMessageText = "UC-UC: iUserCard - DELETED (Id=" + u_id.ToString() + ")"
                                    });
                                    dbTT.SaveChanges();

                                    Console.WriteLine("UC-UC: iUserCard - DELETED (Id=" + u_id.ToString() + ")");
                                }
                            }
                ...
                strStep = "3"; cutOff = 0;
                Console.WriteLine("UC-UC: iUserCard - Inactive Master Data");
                ...
                if (allActiveiUserCardsResco != null && allActiveiUserCardsResco.Length > 0)
                {
                    using (var db = new ContextUNISDataLogMaster())
                    {
                        ...
                        {
                            var record = db.UNIS_iUserCardsResco.Where(m => m.C_CardNum.Equals(entity)).FirstOrDefault();
                            if (record != null)
                            {
                                if (strUNISRegionID == "2")
                                    record.IsSynced2 = false;
                                else if (strUNISRegionID == "3")
                                    record.IsSynced3 = false;
                                else
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "UC-UC: iUserCard Sync " + strLTitleText + "",
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: iUserCard Sync " + strLTitleText + " - Succeeded! (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + ")"
                        });

                        db.SaveChanges();
                        Console.WriteLine("UC-UC: iUserCard Sync " + strLTitleText + " - Message Logged with Success!");
                    }
                }
                ...
                strStep = "1";
                Console.WriteLine("UC-UC: iUserFinger - Get UNISMaster DB Active Data Only");
                ...
                strStep = "2";
                Console.WriteLine("UC-UC: iUserFinger - INSERT INTO 'UNIS Region' DB");
                        ...
                        foreach (var entity in allActiveiUserFingers)
                        {
                            ...
                            if (dbUNISiUserFinger != null)
                            {
                                ...
                                using (var dbTT = new DLL.Models.Context())
                                {
                                    dbTT.log_message.Add(new DLL.Models.LogMessage
                                    {
                                        LogTitle = "UC-UC: iUserFinger - DELETED " + strLTitleText + "",
                                        LogDateTime = DateTime.Now,
                                        LogMessageText = "UC-UC: iUserFinger - DELETED (Id=" + u_id.ToString() + ")"
                                    });
                                    dbTT.SaveChanges();

                                    Console.WriteLine("UC-UC: iUserFinger - DELETED (Id=" + u_id.ToString() + ")");
                                }
                            }
                ...
                strStep = "3"; cutOff = 0;
                Console.WriteLine("UC-UC: iUserFinger - Inactive Master Data");
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "UC-UC: iUserFinger - Sync " + strLTitleText,
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: iUserFinger - Sync " + strLTitleText + " - Succeeded! (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + ")"
                        });

                        db.SaveChanges();
                        Console.WriteLine("UC-UC: iUserFinger - Sync " + strLTitleText + " - Message Logged with Success!");
                    }
                }
                */
                #endregion
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "UC-UC: Completed",
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: All Steps Completed"
                        });
                        db.SaveChanges();
                    }
                }

                Console.WriteLine("UC-UC: ALL Steps Completed!");
            }
            catch (Exception ex)
            {
                ...
                if (isTTLogAllowed)
                {
                    using (var db = new DLL.Models.Context())
                    {
                        db.log_message.Add(new DLL.Models.LogMessage
                        {
                            LogTitle = "UC-UC: Exception " + strLTitleText,
                            LogDateTime = DateTime.Now,
                            LogMessageText = "UC-UC: Exception " + strLTitleText + " - At STEP: " + strStep + " (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + "), StackTrace: " + stkText + ", Message: " + msgText + ", Inner: " + innText
                        });

                        db.SaveChanges();
                        Console.WriteLine("UC-UC: Exception " + strLTitleText + " - At STEP: " + strStep + " (Get=" + count1 + ", Ins=" + count2 + ", Set=" + count3 + "), StackTrace: " + stkText + ", Message: " + msgText + ", Inner: " + innText);
                    }
                }
            }
